...
for i_plot = 1:plot_number
    ...
    plot(Time,plot_Data,Data{i_plot}.color,'LineWidth',3);
...
%         if length(Data{i}.stat_stime)
        ...
        if ( rem(plot_number,2) | (i_plot <= (plot_number)/2) )
            ...
            line( Time( stat_stime(1:8:size(stat_stime,2)) ),48 - i_plot*2,'Marker', 'o','Markersize', 5, 'color',Data{i_plot}.color(1));
        end
        %
